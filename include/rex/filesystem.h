...
// Extracts `archive` (a .zip file) into `dest_dir`, creating it if needed.
// Rejects any entry with an absolute path or a ".." path segment (path
// traversal) -- archives handled here come off the network (catalog mod
// installs), so this must never be trusted to stay inside `archive`'s own
// top-level names. Returns false and populates `error` on any failure
// (corrupt archive, path traversal attempt, filesystem error); never throws.
