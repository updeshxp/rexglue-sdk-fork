...
class WmaPlayer {
 public:
  ...
  // |start_index|. When the final song finishes it is repeated, reproducing the
  // intro+loop behaviour titles get from XMP title playlists. Replaces any
  // current playback. Returns false if no song could be parsed/decoded.
  bool PlayPlaylist(std::vector<std::vector<uint8_t>> songs, size_t start_index);
