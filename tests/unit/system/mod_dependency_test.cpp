...
TEST_CASE(
    "mod dependency validation: requires warns but still succeeds when the dependency is missing",
    "[mod_dependency]") {
  ...
  WriteMod(mods_root, "ui_color", /*requires_list=*/"game_symbols");
  // game_symbols is never enabled. Only version mismatches are fatal; a
  // missing dependency is logged but doesn't stop Setup().

  CHECK(SetupWithMods(game_root, mods_root, "ui_color"));
}

TEST_CASE(
    "mod dependency validation: requires warns but still succeeds when the dependency loads "
    "after",
    "[mod_dependency]") {
  ...
  // ui_color listed before game_symbols: wrong order. Logged, not fatal.
  CHECK(SetupWithMods(game_root, mods_root, "ui_color,game_symbols"));
}

TEST_CASE("mod dependency validation: conflicts warns but still succeeds regardless of order",
          "[mod_dependency]") {
  ...
  CHECK(SetupWithMods(game_root, mods_root, "ui_color,legacy_hack"));
  CHECK(SetupWithMods(game_root, mods_root, "legacy_hack,ui_color"));
}
